 ...
 */
package com.chenws.iot.transport.netty.mqtt.topic;
...
class CNode {
    ...
    //Copy constructor
    private CNode(Token token, List<INode> children, Set<SubscribeBO> subscriptions) {
    ...
    CNode addSubscription(SubscribeBO newSubscription) {
        // if already contains one with same topic and same client, keep that with higher QoS
        if (subscriptions.contains(newSubscription)) {
            final SubscribeBO existing = subscriptions.stream()
                .filter(s -> s.equals(newSubscription))
                .findFirst().get();
            if (existing.getMqttQoS() < newSubscription.getMqttQoS()) {
                subscriptions.remove(existing);
                subscriptions.add(new SubscribeBO(newSubscription));
            }
        } else {
            this.subscriptions.add(new SubscribeBO(newSubscription));
        }
        return this;
    }
    ...
    boolean containsOnly(String clientId) {
        for (SubscribeBO sub : this.subscriptions) {
    ...
    void removeSubscriptionsFor(String clientId) {
        Set<SubscribeBO> toRemove = new HashSet<>();
        for (SubscribeBO sub : this.subscriptions) {
            if (sub.getClientId().equals(clientId)) {
                toRemove.add(sub);
            }
